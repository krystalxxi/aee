...
public class GTMSubsysEventGenerator extends EventGenerator
{
    ...
    public GTMSubsysEventGenerator() {
        this.database = null;
        this.selectSQL = "SELECT T.* FROM TL_GTM_WORKINST T WHERE T.WORK_STATUS = '0' AND T.FIRE_EXEC_TIME < SYSDATE";
    ...
    public Object[] generate() {
        ...
        String sSQL = null;
        if (this.subsys != null && this.subsys.trim().length() > 0) {
            sSQL = "SELECT s1.*,@rowno:=@rowno+1 AS ROWNUM FROM (" + this.selectSQL + " AND UPPER(T.WORK_ARGU) LIKE '{SUBSYS=" + this.subsys.trim() + ",%' ORDER BY T.FIRE_EXEC_TIME) s1,(SELECT @rowno:=0) s2 WHERE @rowno < 30";
        }
        else {
            sSQL = this.selectSQL;
        }
        AEEPlatform.getInstance().getLogger().debug((Object)("sSQL:" + sSQL));
